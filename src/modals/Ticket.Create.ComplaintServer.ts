...
const modal: Modal = {
    customId: "ticket.create.complaint.server",
    execute: async (interaction: ModalSubmitInteraction) => {
        await interaction.deferReply({ ephemeral: true })

        const nick = interaction.fields.getTextInputValue("nickInput")
        const date = interaction.fields.getTextInputValue("dateInput")
        const steamId = interaction.fields.getTextInputValue("steamInput")
        const details = interaction.fields.getTextInputValue("detailInput")

        const channelName = `сервер_жалоба-${interaction.user.username.toLowerCase().slice(0, 15)}`
        ...
        const notifyEmbed = new EmbedBuilder()
            .setTitle("📥 Новая жалоба на сервере")
            .addFields(
