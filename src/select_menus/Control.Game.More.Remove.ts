...
const selectMenu : SelectMenu = {
    ...
    execute: async (interaction) => {
        ...
        let newarray = []
        for (let i = 0; i < game.more.length; i++) {
            const page = game.more[i]
            
            const find = interaction.values.find(value => Number(value) === i)
            if (!find) {
                newarray.push(page)
            }
        }
        ...
        await interaction.update({content: "Готово!", embeds: [], components: []})
    }
}
