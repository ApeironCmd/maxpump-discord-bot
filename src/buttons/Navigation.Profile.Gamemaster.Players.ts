...
const button : Button = {
    customId: "navigation.profile.gamemaster.players",
    execute: async (interaction, gameID, page = "0") => {
        ...
        if (usersOnPage.length === 0 && currentPage > 0) {
            return button.execute(interaction, gameID, "0")
        }
        ...
        const row1 = new ActionRowBuilder<StringSelectMenuBuilder>()
            .addComponents(
                new StringSelectMenuBuilder()
                    .setCustomId(`navigation.profile.gamemaster.players.select/${gameID}`)
        ...
        const row2 = new ActionRowBuilder<ButtonBuilder>()
            .addComponents(
                new ButtonBuilder()
                    .setCustomId(`navigation.profile.gamemaster.players/${gameID}/${currentPage - 1}`)
                    .setLabel("Предыдущая")
                ...
                new ButtonBuilder()
                    .setCustomId(`navigation.profile.gamemaster.select/${gameID}`)
